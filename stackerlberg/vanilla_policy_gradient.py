...
def train_one_epoch(
    ...
    test_fixed_policy = lambda obs : torch.tensor([0 for s in obs],dtype=torch.int).squeeze()
    # test_fixed_policy = lambda obs : torch.tensor([1],dtype=int)
    ...
    while True:
        ...
        random_leader_responses = test_fixed_policy(obs=INIT_LEADER_OBS)
        ...
        for timestep in range(episode_timesteps):
            ...
            action["leader"] = test_fixed_policy(obs=torch.tensor([observation["leader"]]))
